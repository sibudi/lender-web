...
<template>
  <div class="mt_100">
    <div class="scatted_bar_bg">
      <div class="df aic scatted_bar">
        <i class="el-icon-warning" style="color:#FFC018;margin-right:10px;font-size:16px;"></i>
        <p>
          {{selectCount}} claim telah dipilih, total:
          <span class="colC9">Rp：{{$globals.dataUtil.parseNumber(selectAmount)}}</span>
        </p>
      </div>
    </div>
    <!--内容-->
    <div class="order_con">
      ...
      <div class="order_box order_table">
        ...
        <div class="search_tit" style="position: relative;">
          ...
          <el-dropdown style="position: absolute;right: 20px;top: 4px;">
            ...
            <el-dropdown-menu slot="dropdown" class="table_left_drop">
              <el-dropdown-item>
                <p @click="handleSort('biddingTime','DESC')">Susunan Waktu Rilis Terbalik</p>
              </el-dropdown-item>
              <!-- 时间倒序 -->
              <el-dropdown-item>
                <p @click="handleSort('biddingTime','ASC')">Susunan Waktu Rilis Normal</p>
              </el-dropdown-item>
              <!-- 时间正序 -->
              <el-dropdown-item>
                <p
                  @click="handleSort('borrowerYearRate','DESC')"
                >Rating Pengembalian Terbalik Tahunan</p>
              </el-dropdown-item>
              <!-- 收益到序 -->
              <el-dropdown-item>
                <p @click="handleSort('borrowerYearRate','ASC')">Rating Pengembalian Normal Tahunan</p>
              </el-dropdown-item>
              <!-- 收益正序 -->
            </el-dropdown-menu>
          </el-dropdown>
          ...
          <div class="search_item df aic">
            ...
            <div class="block" style="width: 228px;">
              <el-slider
                ...
                :min="18"
                :max="50"
                v-model="checkAgelist"
                @change="handleCheck"
              ></el-slider>
              <div class="df jcsb age_marks">
                <span>18</span>
                <span>50</span>
              </div>
            </div>
        ...
        <div v-for="item in tableData" class="df aic ele_check" style="padding-left: 15px;">
          ...
          <ul class="df table_box jcsb f1 ml_25" @click="goDetail(item.creditorNo)">
            <li style="wrap-text:break-word;width:180px">
              <p class="table_item_tit">Tujuan Pinjaman</p>
              <p class="text-center list-detail ">
                {{item.borrowingPurposes}}
              </p>
            </li>

            <li>
              <p class="table_item_tit">Jumlah pinjaman</p>
              <p class="list-detail">Rp {{$globals.dataUtil.parseNumber(item.amountApply)}}</p>
            </li>
            <li>
            ...
            <li style="wrap-text:break-word;width:100px">
              ...
              <p class="text-center list-detail ">{{getLabelledTerm(item.term)}}</p>
            </li>
            <li>
...
export default {
  ...
  methods: {
    handleCheckedChange(item) {
      ...
      if (item.checked) {
        let amount = parseFloat(
          parseFloat(item.amountApply) -
            parseFloat(item.amountBuy) +
            parseFloat(item.insurance)
        );
        if (
          parseFloat(_this.selectAmount) + parseFloat(amount) <
          parseFloat(_this.currentBalance)
        ) {
          _this.selectCount = _this.selectCount + 1;
          _this.selectAmount =
            parseFloat(_this.selectAmount) + parseFloat(amount);
          _this.selectData.push({
            goodsId: item.creditorNo,
            item: item,
            amount:
              ((parseFloat(item.amountApply) - parseFloat(item.amountBuy)) /
                parseFloat(item.amountApply)) *
              100
          });
      ...
      if (_this.tableAmount < _this.currentBalance) {
        ...
        if (_this.isAllList) {
          _this.tableData.forEach(v => {
            v.checked = true;
            _this.selectCount++;
            _this.selectAmount += parseFloat(
              parseFloat(v.amountApply) -
                parseFloat(v.amountBuy) +
        ...
        _this.tableData.forEach(v => {
          let amount = parseFloat(
            parseFloat(v.amountApply) - parseFloat(v.amountBuy)
          );
          if (
            parseFloat(temp) + parseFloat(amount) <
            parseFloat(_this.currentBalance)
          ) {
    ...
    fetchData() {
      ...
      _this.$axios
        .post("/api-order/scatterStandard/selectScatterstandardList", _data)
        .then(function(re) {
          if (re.data.code == 0) {
            ...
            _this.tableData.forEach(v => {
              ...
              v.percentage = parseFloat(
                (
                  (parseFloat(v.amountBuy) / parseFloat(v.amountApply)) *
                  100
                ).toFixed(2)
              );
              if (_this.isAllList) {
            ...
            if (!_this.isAllList) {
              //若未选择剩余全买，则比较之前是否选择过该债权
              for (let j = 0; j < _this.selectData.length; j++) {
                for (let i = 0; i < _this.tableData.length; i++) {
                  if (
                    _this.tableData[i].creditorNo == _this.selectData[j].goodsId
                  ) {
    ...
    addCartAll() {
      ...
      if (!_this.isAllList && _selectData.length == 0) {
        _this.$message({ type: "warning", message: "Silakan memilih claim" });
        return;
      }
      ...
      let _url = _this.isAllList
        ? "/api-order/shoppingCart/addAll"
        : "/api-order/shoppingCart/addList";
