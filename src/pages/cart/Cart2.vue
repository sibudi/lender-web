  ...
  <div class="mt_100">
    <div class="cart_del_bar" v-if="fullNumber>0">
      <div class="cart_del_info df aic">
        ...
        <p>
          Ada {{fullNumber}} target yang sudah penuh,
          <span
            class="colC9"
            @click="handleDel"
          >hapus sekali klik</span>
        </p>
    ...
    <div class="cart_con" v-if="tableData.length>0">
      ...
      <div class="order_box order_table">
        <div v-for="item in tableData" :key="item.goodsId">
          <ul class="df table_item jcsb">
            ...
            <div class="cart_full" v-if="item.count==0&&item.count !=''&&!item.focus">
              <p class="full_text">Penuh</p>
            </div>
          </ul>
        </div>
      </div>
    ...
    <div class="cart_topay_bar" v-if="tableData.length>=0">
      <div class="cart_topay_con df aic">
        ...
        <div class="cart_pay_btn df aic" @click="handlePay">
  ...
  methods: {
    ...
    handleBuyAll() {
      ...
      if (_this.allIn) {
        ...
        _this.$axios
          .post("/api-order/shoppingCart/resetCart", {})
          .then(function(re) {
            loading.close();
            if (re.data.code == 0) {
        ...
        _this.tableData.forEach(v => {
          console.log(
            parseInt(v.totalAmount.split(".")[0]),
            parseInt(v.laveAmount.split(".")[0])
          );
          v.count =
            ((parseInt(v.totalAmount.split(".")[0]) -
              parseInt(v.laveAmount.split(".")[0])) /
    ...
    buyAll(item) {
      ...
      item.count = ((_totalAmount - _laveAmount) / _totalAmount) * 100;
    ...
    handlePay() {
      ...
      // let _amount = 0;
      // _this.tableData.forEach(v => {
      //   _amount = _amount + v.totalAmount * (v.count / 100);
      // });
      // let _data = {
      ...
      //     "Mohon maaf pendanaan yang anda ajukan telah penuh"
      //   );
      // }
      // _this.$axios
    ...
    handleDel() {
      ...
      _this.$axios
        .post("/api-order/shoppingCart/deleteFull", {
    ...
    handleOneDel(goodsId) {
      ...
      _this.$axios
        .post("api-order/shoppingCart/deleteOne", {
          userId: _this.$store.getters.userUuid,
          goodsId: goodsId
        })
        .then(function(re) {
    ...
    handleCount() {
      ...
      _this.tableData.forEach(v => {
        _amount +=
         (parseFloat(v.totalAmount)+ parseFloat(v.insurance)) 
        //  *v.count/100
         ;
      });
    ...
    changeCutNum: async function(goodsId, amount) {
      let _this = this;
      _this.$axios
        .post("api-order/shoppingCart/updateAmount", {
          userId: _this.$store.getters.userUuid,
          goodsId: goodsId,
          amount: amount
        })
    ...
    handleBlur(item) {
      ...
      let _laveAmount = parseInt(item.laveAmount.split(".")[0]);
      let _totalAmount = parseInt(item.totalAmount.split(".")[0]);
      let _maxCount = ((_totalAmount - _laveAmount) / _totalAmount) * 100;
    ...
    cutNumber: async function(num, index, goodsId) {
      ...
      let number = num - 10 > 10 ? num - 10 : 10;
    ...
    addNumber: async function(item) {
      ...
      let number = parseInt(item.count) + 10;
      item.count = number;
      _this.changeCutNum(item.goodsId, item.count);
    },
    // 剩余金额
