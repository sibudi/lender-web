...
<div class="rights_detail_box">
  ...
  <div class='ele_table'>
    <div class="right_detail_tit df aic jcsb">
      ...
      <p>Waktu Pembuatan：{{rights.createTime}}</p>
    </div>
    <el-table :data="tableData"style="width: 100%" class="rights_list">
      ...
      <el-table-column prop="" label="Jumlah Tunggakan">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.amountApply)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="" label="Tingkat pengembalian tahunan yang diharapkan">
        <template slot-scope="scope">
          {{(parseFloat(scope.row.yearRateFin)*100).toFixed(2)}}%
        </template>
      </el-table-column>
...
export default{
  ...
  methods: {
    ...
    bindData(){
      ...
      _this.$axios.post('/api-order/order/selectOrderDetail', _data).then(function (re) {
        if(re.data.code==0){
...
.rights_detail_box .ele_table{
  ...
  .status_quares1,.status_quares2,.status_quares3,.status_quares4,.status_quares9{
    background-color: #F9CF2D
  }
  .status_quares6,.status_quares7,.status_quares8,.status_quares10,.status_quares11,.status_quares12,.status_quares13{
