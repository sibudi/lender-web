...
<div>
  ...
  <div class="df asset_box jcsb">
    <div class="asste_item">
      <div class="asste_top"><p class="asset_top_tit">Jumlah investasi</p><p>Rp{{$globals.dataUtil.parseNumber(investingBanlance)}}</p></div>
      <div class="asste_tips">Kunci aset Rp{{$globals.dataUtil.parseNumber(lockedBalance)}}</div>
    </div>
    <div class="asste_item">
      <div class="asste_top"><p class="asset_top_tit">Aset yang tersedia</p><p>Rp{{$globals.dataUtil.parseNumber(currentBalance)}}</p></div>
    ...
    <div class="df aic table_search">
      ...
      <div>         
        <el-select v-model="businessType" clearable class="form_select search_sel" >
          <el-option v-for="item in businessTypeList" :key="item.value" :label="item.label" :value="item.label"></el-option>
        </el-select>
      </div>
      <div><button class="serach_btn" @click="handleSectet">Ajukan</button></div>
    </div>
    <el-table :data="tableData"style="width: 100%" class="asste_list">
      <el-table-column prop="businessType" label="Mengetik"></el-table-column>
      <el-table-column prop="dealTime" label="Memindahkan waktu akun" width="190"> 
          <template slot-scope="scope">{{scope.row.dealTime.split('.')[0]}}</template>
      </el-table-column>
      <el-table-column prop="amount" label="Jumlah uang"  width="110">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.amount)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="currentBanlance" label="Saldo akun yang tersedia"  width="190">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.currentBanlance)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="investBanlance" label="Saldo akun investasi" width="160">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.investBanlance)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="lockedBanlance" label="Kunci saldo akun"  width="140">
        <template slot-scope="scope">
...
export default{
  ...
  data(){
    return {
      ...
      businessTypeList: [{value: '1',label: 'Isi ulang'},{value: '2',label: 'Pembelian Kreditor'},{value: '3',label: 'Klaim Pembelian Sukses'},{value: '4',label: 'Klaim Pembelian Gagal'},{value: '5',label: 'Pendapatan bunga'},{value: '6',label: 'Biaya admin 75%'},{value: '7',label: 'Pengembalian Dana'},{value: '8',label: 'Pendaftaran Penarikan Dana'},{value: '9',label: 'Penarikan Berhasil'},{value: '10',label: 'Dana dikembalikan'}],
      businessType:''
    }
  },
  async mounted(){
  ...
  methods: {
    ...
    fetchData(){
      ...
      _this.$axios.post('/api-user/user/getAccountHistoryByType', _data).then(function (re) {
        if(re.data.code==0){
