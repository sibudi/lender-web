...
<div>
  ...
  <div class='ele_table'>
    <el-table :data="tableData" class="records_list">
      <el-table-column prop="id" label="Nomor order"></el-table-column>
      <el-table-column prop="createTime" label="Waktu Pembuatan"> </el-table-column>
      <el-table-column prop="applyBuy" label="Jumlah Pinjaman" width="135px">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.applyBuy)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="status" label="Status pemesanan"  width="155px">
        <template slot-scope="scope">
          <span><i class="status_quare" :class="setClass(scope.row.status)"></i>{{getOrderStatus(scope.row.status)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="c" label="Perlu mengisi ulang jumlah">
        <template slot-scope="scope">
          <span>Rp {{$globals.dataUtil.parseNumber(scope.row.chargeBuy)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="buyTime" label="Waktu pembayaran"></el-table-column>
      ...
      <el-table-column label="Terapkan" width="160px">
        <template slot-scope="scope">
          <div class="df record_operate">
            ...
            <p  v-if="scope.row.status ==1|| scope.row.status ==3" @click="goDetail(scope.row)">Lihat klaim</p>
            <p  v-if="scope.row.status ==2 || scope.row.status ==9"  style="color:#8F9DAA;">Validasi klaim</p>
          </div>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getOrderStatus(status){
      // let list = [{code:'1',name:'投资处理中'},{code:'2',name:'投资失败'},{code:'3',name:'投资成功'},{code:'9',name:'订单失效'}];
       let list = [{code:'1',name:'Pemrosesan investasi'},{code:'2',name:'Kegagalan investasi'},{code:'3',name:'investasi berhasil'},{code:'9',name:'Pesanan tidak valid'}];
      let re = '';
