  ...
  <div class="indextop">
    <div class="navBar">
      <div class="topBar df aic">
        <div style="margin-left: 0px;" class="logo_icon f1">
          <img src="../assets/images/Doitnavbar.png" width="42px" alt />
        </div>
        <div class="navBarContent">
          <ul class="df">
            ...
            <li v-for="item in urlLists" v-if="pathConfig!=='/authentication'">
              <a
                @click="a(item.path)"
                :class="thisClass==item.path? 'currentClass':''"
                v-if="item.link"
              >{{item.name}}</a>
              <el-dropdown v-else class="el_li">
                ...
                <el-dropdown-menu slot="dropdown" class="nav_left_drop">
                  <el-dropdown-item>
                    <router-link to="/aboutUs">Tentang kami</router-link>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <router-link to="/blogList">Blog</router-link>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <router-link to="/joinus">Karir</router-link>
                  </el-dropdown-item>
                  <el-dropdown-item>
        ...
        <div class="login_state df aic">
          <div class="tkb_rate df aic">
            ...
            <div
              class="tkb_rate_tips"
              v-if="showRateTips"
            >Tingkat keberhasilan Penyelenggara Layanan Pinjam Meminjam Uang Berbasis Teknologi Informasi dalam memfasilitasi penyelesaian kewajiban pinjam meminjam antara Penerima Pinjaman kepada Pemberi Pinjaman dalam jangka waktu sampai dengan 90 hari terhitung sejak jatuh tempo.</div>
            <img
          ...
          <div class="df aic" v-show="currentUser">
            ...
            <el-dropdown>
              <span style="color: rgba(22,66,118,1);" class="el-dropdown-link">
                {{userName?userName:currentUser}}
                <!-- <i class="el-icon-arrow-down el-icon--right"></i> -->
              </span>
              <el-dropdown-menu slot="dropdown" class="nav_left_drop">
                <el-dropdown-item v-if="pathConfig!=='/authentication'">
                  <a @click="pushto(1)">Pusat pribadi</a>
                </el-dropdown-item>
                <el-dropdown-item>
                  <a @click="pushto(2)">Keluar</a>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
            <img
              src="../assets/images/cart_icon01.png"
              height="22"
              width="24"
              class="cart_icon"
              @click="goCart"
              v-if="pathConfig!=='/authentication'"
            />
            <div class="cart-badge" v-if="cartCount &&pathConfig!=='/authentication'">{{cartCount}}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      avatar:'',
        ...
        // process.env.SHOW_IMAGE +
        // "?path=/mnt/MyUpload/img/w/k/g/98328763f4fe45f7b7b8ae9f48dfddab.JPG&sessionId=c7e8ff5978654af1a662d2809e76ef4f",
      showRateTips: false,
      ...
      bannerLists: [
        {
          ...
          text: "Do-it Untuk Masa Depan yang Lebih"
        },
        { url: "../../static/banner1.png" }
      ],
      urlLists: [
        { name: "Pendanaan", path: "Scattered", link: true }
      ...
      note: {
        backgroundImage:
          "url(" + require("../assets/img/alertContent.png") + ")",
  ...
  methods: {
    ...
    init() {
      ...
      this.avatar =
        _this.$store.getters.headImage == ""
          ? "../assets/images/touxiang.png"
          : _this.$store.getters.headImage;
      this.$axios
        .post("/api-order/order/getRepayRate", {})
        .then(response => {
    ...
    getCart() {
      axios({
        method: "post",
        url: process.env.BASE_URL + "api-order/shoppingCart/getCartCount",
        data: {
          sessionId: this.$store.getters.token,
          sessionId: this.$store.getters.userUuid
        }
      })
        .then(res => {
  ...
  computed: mapGetters(["currentUser", "userName", "headImage"]),
  watch: {
    $route(to, from) {
      this.pathConfig = to.path
      if (this.$store.getters.token) {
        this.getCart();
      }
    }
